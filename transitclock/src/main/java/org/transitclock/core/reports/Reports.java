...
private static final int MAX_NUM_DAYS = 7;
	...
	public static String getTripsFromArrivalAndDeparturesByDate(String agencyId, String date) {		
		...
		String sql = "SELECT "
				...
				+ "WHERE Date(arrivalsdepartures.time) = DATE('" + date + "') "
	...
	public static String getTripWithTravelTimes(String agencyId, String tripId,
			String date) {		
		//postgresql only, should throw error if it's other database type
		String sql = "SELECT"
				+ "	arrivalsdepartures.tripid as tripId"
				...
				+ "WHERE arrivalsdepartures.tripid = '" + tripId + "' and arrivalsdepartures.isarrival = 'True' and Date(arrivalsdepartures.time) = DATE('" + date + "')"
				+ "ORDER BY arrivalsdepartures.time asc, arrivalsdepartures.directionid asc, arrivalsdepartures.gtfsstopseq asc";		
    ...
    Request parameters are:
      ...
      allowableLateMinutes - how early vehicle can be and still be OK. Decimal format OK.
	 */
	public static String getScheduleAdhByStops_v2(String agencyId, String route, String beginDate,
			String allowableEarly, String allowableLate, String beginTime, String endTime, int numDays) {
		if (allowableEarly == null || allowableEarly.isEmpty())
			allowableEarly = "1.0";
		String allowableEarlyMinutesStr = "'" + SqlUtils.convertMinutesToSecs(allowableEarly) + " seconds'";
		...
		String sql_trips_early =
				"SELECT tripid AS trips_early, "
						+ "	 regexp_replace(CAST(DATE_TRUNC('second', ad.scheduledTime::timestamp) - DATE_TRUNC('second', ad.time::timestamp) AS VARCHAR), '^00:', '') difference_in_seconds, \n"
						+ "	 s.id AS stop_id, \n"
						+ "	 ad.stopOrder AS stop_order \n"
						+ " 	FROM ArrivalsDepartures ad, Stops s  \n"
		...
		String sql_trips_late = "SELECT tripid AS trips_late,  "
				+ "	 regexp_replace(CAST(DATE_TRUNC('second', ad.time::timestamp) - DATE_TRUNC('second', ad.scheduledTime::timestamp) AS VARCHAR), '^00:', '') difference_in_seconds, \n"
				//+ "	 ((ad.time / 1000) - (ad.scheduledTime / 1000)) AS difference_in_seconds,  \n"
				+ "	 s.id AS stop_id, \n"
				+ "	 ad.stopOrder AS stop_order \n"
